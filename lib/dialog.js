...
Dialog.prototype = {
    ...
    render: function(content, style, id) {
        if(!this._frameId) this._setFrame(this.timestamp);
    ...
    _frameVisible: function(v) {
        if (!arguments.length) return this._frameId && this.constructor.GetStyle(document.getElementById(
    ...
    _setFrame: function(timestamp) {
        if (!(this.parentEl) || this._frameId) return;

        this._frameId = 'DialogFrame_' + (timestamp ? timestamp : (new Date().getTime()).toString()); // 时间戳
