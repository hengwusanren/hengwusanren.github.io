var ScrollControl = function(el, callback) {
    ...
    page.addEventListener('touchend', function(e) {
        ...
        var initialSpeed = ScrollHelp.calSpeed(distYIntervals, timeIntervals);
...
var ScrollHelp = {
    ...
    _setTransitionClass: function(el, time, x1, y1, x2, y2, className) {
        if (ScrollHelp[className]) document.getElementsByTagName('head')[0].removeChild(ScrollHelp[className]);
        ScrollHelp[className] = document.createElement('style');
        ScrollHelp[className].type = 'text/css';
        var t = 'transition: all ' + time + 's cubic-bezier(' + x1 + ', ' + y1 + ', ' + x2 + ', ' + y2 + ');';
        t = '-webkit-' + t + t;
        ScrollHelp[className].innerHTML = '.' + className + ' { ' + t + ' }';
        document.getElementsByTagName('head')[0].appendChild(ScrollHelp[className]);
